...
from PyQt5.QtCore import QBuffer, QByteArray, QDataStream, QIODevice, QObject
from PyQt5.QtMultimedia import QAudioDeviceInfo, QAudioFormat, QAudioInput, QAudioOutput
    ...
    def init_audio_output(self):
        ...
        if not info.isFormatSupported(self.audio_format_out):
            ...
            if nearest.sampleRate() != self.audio_format_out.sampleRate():
                print(
                    f"Warning: Device using sample rate {nearest.sampleRate()} Hz instead of requested {self.audio_format_out.sampleRate()} Hz"
                )
        ...
        self.bytes_per_second = (
            self.audio_format_out.sampleRate()

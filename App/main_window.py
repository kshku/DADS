...
from PyQt5.QtCore import Qt, QByteArray, QFile, QIODevice, QDateTime
...
class MainWindow(QWidget):
    ...
    def load_initial_pdfs(self):
        ...
        if not pdf_paths:
            self.status_label.setText(f"Ready. No PDFs found in the passage folder.")
            return
    ...
    def save_recorded_audio(self):
        ...
        if file_path:
            ...
            try:
                self.audio_handler.write_wav_file(file, self.audio_handler.audio_data, 
                                                   self.audio_handler.audio_format_out)
                file_name = file_path.split('/')[-1]

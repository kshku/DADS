    ...
    def progress_callback(self, chunk_idx, total_chunks, chunk_results):
        ...
        for stutter_type in ["prolongation", "soundrep", "wordrep", "block", "interjection"]:
            ...
            for chunk_data in self.chunk_detections.values():
                if (
                    "detections" in chunk_data
                    and stutter_type in chunk_data["detections"]
                    and chunk_data["detections"][stutter_type]["detected"]
                ):
                    count += 1
...
class AnalysisWidget(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.stutter_counts = {"prolongation": 0, "soundrep": 0, "wordrep": 0, "block": 0, "interjection": 0}
    ...
    def _init_ui(self):
        ...
        self.detection_status_label.setStyleSheet(
            "font-size: 14px; padding: 10px; color: #ffa500; background-color: #2a2a2a; border-radius: 4px;"
        )
        ...
        for lbl in [
            self.class_prolongation,
            self.class_soundrep,
            self.class_wordrep,
            self.class_block,
            self.class_interjection,
        ]:
        ...
        self.fwd_btn_5s.clicked.connect(
            lambda: self.seek_to_position(min(self.audio_slider.maximum(), self.audio_slider.value() + 5000))
        )
    ...
    def on_detection_error(self, error_msg):
        ...
        self.detection_status_label.setStyleSheet(
            "font-size: 14px; padding: 10px; color: #ff5555; background-color: #2a2a2a; border-radius: 4px;"
        )
    ...
    def update_playback_progress(self):
        ...
        if state == QAudio.ActiveState and not self.is_seeking:
            ...
            self.current_playback_position_sec = max(0, min(self.current_playback_position_sec, total_time))
    ...
    def export_report(self):
        ...
        if not self.detection_results:
            QMessageBox.warning(
                self, "No Results", "No detection results available. Please analyze an audio file first."
            )
            return
        ...
        save_path, _ = QFileDialog.getSaveFileName(
            self, "Save Report As", default_path, "Text Files (*.txt);;All Files (*)"
        )
        ...
        try:
            with open(report_path, "w") as f:
                ...
                sorted_stats = sorted(aggregated_stats.items(), key=lambda x: x[1]["confidence"], reverse=True)
            ...
            QMessageBox.information(self, "Report Exported", f"Report successfully exported to:\n{report_path}")
